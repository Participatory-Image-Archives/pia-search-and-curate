<div>
    ...
    <div>
        <div class="p-10 pr-20 space-y-4" x-data>
            <main>
                @if(count($images))
                <div class="text-right mb-2">
                    <span class="inline-block text-gray-500 text-xs">{{ count($images) }} results</span>
                </div>
                @endif
            ...
            <div>
                ...
                <div>
                    @foreach ($selection as $image)
                        <div class="mt-4" wire:key="selected_{{ $image['id'] }}"
                            wire:click="forget({{ $image['id'] }})">
                            <img class="w-full"
